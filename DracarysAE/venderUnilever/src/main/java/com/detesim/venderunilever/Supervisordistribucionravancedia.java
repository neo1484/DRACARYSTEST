...
public class Supervisordistribucionravancedia extends Activity 
{
	LinearLayout llSupervisorAvanceDistribucionDia;
	Utilidades theUtilidades = new Utilidades();
	MyLogBLL myLog = new MyLogBLL();
	LoginEmpleado loginEmpleado;
	
	ArrayList<AvanceDistribucion> listadoAvanceDistribucionDia;
	
	ListView lvAvanceDistribucionDia;
	...
	protected void onCreate(Bundle savedInstanceState) 
	{
		...
		setContentView(R.layout.activity_supervisoravancedistribuciondia);
		
		llSupervisorAvanceDistribucionDia = (LinearLayout)findViewById(R.id.llDisenioAvanceDistribuidorDia);
		lvAvanceDistribucionDia = (ListView)findViewById(R.id.lvAvanceDistribucionMesDistribuidores);
		...
		llSupervisorAvanceDistribucionDia.setBackground(getResources().getDrawable(theUtilidades.get_fondo()));
	...
	private void MostrarControles(boolean estado)
	{
		...
		lvAvanceDistribucionDia.setVisibility(visibility);
	...
	{
	    listadoAvanceDistribucionDia = null;
	    try
	    {
	    	listadoAvanceDistribucionDia = new AvanceDistribucionBLL().ObtenerAvanceDistribucionPorTipoAvanceDistribuidorYRol("DIA","Supervisor");
	    }
	    catch (Exception localException)
	    {
	    	if(localException.getMessage() == null || localException.getMessage().isEmpty())
	    	{
	            myLog.InsertarLog("App",this.toString(),1,"Error al obtener los avancesDistribucion por dia: vacio");
	    	}
	    	else
	    	{
	    		myLog.InsertarLog("App",this.toString(),1,"Error al obtener los avancesDistribucion por dia: " + localException.getMessage());
	    	} 
	    }
	      
	    if(listadoAvanceDistribucionDia == null)
	    {
	    	MostrarControles(false);
	    	lvAvanceDistribucionDia.setAdapter(null);
	    	theUtilidades.MostrarMensaje(getApplicationContext(), "No existen avances de distribucion en el dia.", 1);
	        return;
	    }
	    else
	    {
	...
	private void LlenarListViewAvanceDistribucion()
	{
	    ...
	    localLayoutParams.height = ((int)(50*getApplicationContext().getResources().getDisplayMetrics().density) * listadoAvanceDistribucionDia.size());
	...
	class MiAdaptadorLista extends ArrayAdapter<AvanceDistribucion>
	{
		...
		{
			super(context,R.layout.disenio_supervisoravancedistribuidor,listadoAvanceDistribucionDia);
		...
		public View getView(int position, View convertView, ViewGroup parent)
		{
			...
			AvanceDistribucion localAvanceDistribucion = (AvanceDistribucion)listadoAvanceDistribucionDia.get(position);
